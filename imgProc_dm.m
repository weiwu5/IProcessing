...
%          Output perimeter, rectangle length/width and eclispe length/width
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
...
for i=((n-1)*nEvery+1):min(n*nEvery,handles.img_count)
    ...
    while data(j,1) ~= -1 && j < size_mat
        % Calculate every particles
        if (isequal(data(j,1), boundary) && (isequal(data(j+1,1), boundarytime) || probetype==1) )
           ...
           if start ==0
               ...
               if probetype==0
                ...
                rec_date(kk)=handles.year(i)*10000+handles.month(i)*100+handles.day(i);
                ...
                %% Get diameter of the smallest-enclosing circle
                %images.minR(kk)=particlesize_opencv(c);
                ...
                images.Perimeter=bwperim(c);

                [images.RectangleL, images.RectangleW]=particleRectangle_opencv(c);
                [images.EllipseL, images.EllipseW]=particleEllipse_opencv(c);
    ...
    if kk > 1
        ...
        netcdf.putVar ( f, varid12, wstart, w-wstart+1, images.image_area);                           
        ...
        netcdf.putVar ( f, varid26, wstart, w-wstart+1, images.minR);                      
        netcdf.putVar ( f, varid27, wstart, w-wstart+1, images.AreaR);       
        netcdf.putVar ( f, varid45, wstart, w-wstart+1, images.Perimeter);         
        netcdf.putVar ( f, varid46, wstart, w-wstart+1, images.RectangleL);                      
        netcdf.putVar ( f, varid47, wstart, w-wstart+1, images.RectangleW);         
        netcdf.putVar ( f, varid48, wstart, w-wstart+1, images.EllipseL);                      
        netcdf.putVar ( f, varid49, wstart, w-wstart+1, images.EllipseW); 
        ...
        wstart = w+1;
        kk = 1;
        clear rec_time rec_millisec part_hour part_min part_sec part_mil part_micro parent_rec_num particle_num images
    end
    clear images
end
warning on all
